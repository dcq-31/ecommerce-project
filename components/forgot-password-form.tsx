...
export function ForgotPasswordForm({
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      // Supabase appends token_hash + type to this URL. /auth/confirm calls
      // verifyOtp to establish the session, then forwards to update-password.
      // Ensure this URL is listed in: Supabase dashboard → Auth → URL Configuration → Redirect URLs
  ...
  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      {success ? (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-xl sm:text-2xl">Restablecer contraseña</CardTitle>
            <CardDescription>
